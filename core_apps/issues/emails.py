...
def send_issue_confirmation_email(issue: Issue) -> None:
    try:
        ...
        logger.error(
            ...
            f"the issue: {issue.title}: {e}, exe_info=True"
        )
...
def send_issue_resolved_email(issue: Issue) -> None:
    try:
        ...
        logger.error(
            f"Failed to send resolution email for"
            f"the issue: {issue.title}: {e}, exe_info=True"
        )
...
def send_resolution_email(issue: Issue) -> None:
    try:
        ...
        from_email = DEFAULT_FROM_EMAIL
        receipient_list = [issue.reported_by.email]
        context = {"issue": issue, "site_name": SITE_NAME}
        html_email = render_to_string(
            "emails/issue_resolved_notification.html",
            context
        ...
        logger.error(
            f"Failed to send issue resolution email for"
            f"the issue: {issue.title}: {e}, exe_info=True"
        )