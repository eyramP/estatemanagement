    ...
    def perform_create(self, serializer: IssueSerializer):
        ...
        try:
            ...
            raise PermissionDenied(
                "You do not have the permisson to raise an issue for this apartment, its not yours"
            )
...
class IssueUpdateAPIView(generics.UpdateAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        if not (user.is_staff or user == issue.assigned_to):
            logger.warning(
                f"Unauthorized issue status update "
                f"attempt by the user {user.get_full_name}"
                f"on issue {issue.title}"
            )
...
class IssueDeleteAPIView(generics.DestroyAPIView):
    ...
    def get_object(self):
        ...
        if not (user == issue.reported_by or user.is_staff):
            logger.warning(
                f"Unauthorized delete attempt by user {user.get_full_name}"
                f"on issue {issue.title}"
            )
