...
class UnBookmarkPostAPIView(APIView):
    def patch(self, request, slug):
        ...
        if user not in post.bookmarked_by.all():
            return Response(
                {"message": "You cannot remove a bookmark does not exist."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class DownvotePostAPIView(APIView):
    def patch(self, request, post_id):
        post = get_object_or_404(Post, id=post_id)
        serializer = DownvotePostSerializer(
            post, data={},
...
class TopPostsListAPIView(generics.ListAPIView):
    ...
    renderer_classes = [GenericJSONRenderer]
    permission_classes = [permissions.AllowAny]
    object_label = "top_post"

    def get_queryset(self):
        queryset = Post.objects.annotate(
            replies_count=Count("replies"), view_count=Count("content_views")
        ).order_by("-upvotes", "-view_count", "-replies_count")[:6]
        return queryset


class PostByTagListAPIView(generics.ListAPIView):
    serializer_class = PostByTagSerializer
    renderer_classes = [GenericJSONRenderer]
    permission_classes = [permissions.AllowAny]
    object_label = "post_by_tag"
