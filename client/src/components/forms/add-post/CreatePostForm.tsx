...
import { useCreatePostMutation } from "@/lib/redux/features/post/postApiSlice"
import { postSchema, TPostSchema } from "@/lib/validationSchemas/PostSchema"
...
function CreatePostFormContent() {
    ...
    const onSubmmit = async(values: TPostSchema) => {
        try {
            await createPost(values).unwrap()
            toast.success("You post has been added successfully")
            router.push("/welcome")
