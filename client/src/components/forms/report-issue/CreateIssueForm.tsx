...
export default function CreateIssueForm() {
  ...
  const {register, handleSubmit, control, reset, formState: {errors}} = useForm<TIssueCreateSchema>({
    ...
    const onSubmit = async(formValues: TIssueCreateSchema) => {
        if (!apartment?.id){
            toast.error("Add your apartment in your profile first before creating an issue.");
            return;
        }
        ...
        try {
            ...
            toast.success("Your issue has been reported and a confirmation email has been sent to you.");
            reset();
            router.push("/profile")
        }catch (error){
  ...
  return (
    <main>
        ...
        className="flex w-full max-w-md flex-col gap-4 dark:text-black text-black"
        >
            <FormFieldComponent
            label="Title"
            name="title"
            ...
            startIcon={<FlagIcon className="dark:text-babyPowder text-black size-8"/>}
            />
            <FormFieldComponent
            label="Description"
            name="description"
            ...
            className="bg-babyPowder dark:bg-black"
            />
            <div>
            ...
            <ClientOnly >
                ...
                render={({field:
                    ...
                    <Select className="mt-1 w-full" options={statusOptions} value={statusOptions.find(
            ...
            {errors.status && (<p className="mt-2 text-sm text-red-500">{errors.status.message}</p>)}
            </div>
            <div>
                <label htmlFor="Priority" className="h4-semibold dark:text-babyPowder">Priority</label>
            <div className="mt-1 flex items-center space-x-3 text-sm">
            <ClientOnly >
                <Controller
                name="priority"
                ...
                render={({field:
                    {onChange, onBlur, value}}) => (
                    <Select className="mt-1 w-full" options={priorityOptions} value={priorityOptions.find(
                        (option) => option.value === value
                    )}
                    onChange={(val) => onChange(val?.value)}
                    onBlur={onBlur}
            ...
            {errors.priority && (<p className="mt-2 text-sm text-red-500">{errors.priority.message}</p>)}
            </div>
            <Button
            type="submit"
