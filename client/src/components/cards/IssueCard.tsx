...
export default function IssueCard({issue}: IssueCardProps) {
  return (
    <Link href={`/issues/${issue.id}`} key={issue.id}>
        <Card key={issue.id} className="hover:border-pumpkin border-gray hover:dark:border-platinum rounded-xl border border-dashed">
            <CardHeader >
                <CardTitle className="flex-center h3-semibold font-robotoSlab dark:text-lime-500">
                    {issue.title.length > 20 ? `${issue.title.substring(0, 20)}....`
                    :issue.title}
                </CardTitle>
            </CardHeader>
            <CardContent>
                <CardDescription className="dark_text-platinum">
                <p className="h4-semibold">
                    {issue.description.length > 20 ? `${issue.title.substring(0, 35)}....`
                    :issue.description}
                </p>
                </CardDescription>
            </CardContent>
