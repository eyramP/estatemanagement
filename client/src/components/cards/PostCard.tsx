...
import { useGetAllPostQuery } from "@/lib/redux/features/post/postApiSlice"
import { useAppSelector } from "@/lib/redux/hooks/typedHooks"
import { PostState } from "@/types"
import { formatDate, getRepliesText, getViewText, sortByDateDescending } from "@/utils"
import Spinner from "../shared/Spinner"
import Link from "next/link"
import { Button } from "../ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "../ui/card"
import {formatDistanceToNow, parseISO} from "date-fns"
import { EyeIcon, MessageSquareQuote, MessageSquareQuoteIcon } from "lucide-react"
...
export default function PostCard() {
    ...
    const sortedPosts = sortByDateDescending(
        data?.posts.results ?? [],  "created_at"
    )

    if (isLoading){
        return <div className="flex-center pt-32">
            <Spinner size="xl"/>
        </div>
    }
  ...
  return (
    ...
    <h1 className="font-robotoSlab dark:text-pumpkin">
        All posts - ({data?.posts.results.length})
    </h1>
    <Link href="/add-post" className="flex justify-end max-sm:w-full">
    <Button className="h3-semibold electricIndigo-gradient text-babyPowder min-h-[30px] px-4 py-3">
    Create a Post
    </Button>
    </Link>
    </div>
    ...
    {sortedPosts && sortedPosts.length > 0 ? (
        sortedPosts.map((postItem) => (
            <Card key={postItem.id} className="dark:border-gray rounded-lg border">
                ...
                {postItem.title.length > 25 ? `${postItem.title.substring(0,25)}...` : postItem.title}
                </CardTitle>
                <CardDescription>
                    <div className="flex flex-row justify-between">
                        <div>
                            <span>Posted on </span>
                            <span className="dark:text-pumpkin mr-1">
                    ...
                    <div>
                        ...
                        <span className="dark:text-pumpkin mr-1">
                            {formatDistanceToNow(parseISO(postItem.updated_at), {addSuffix: true})}
                        </span>
                    </div>
                </CardDescription>
                </CardHeader>
                <CardContent className="border-deepBlueGrey dark:border-gray border-y py-4 text-sm">
                    <p className="dark:text-platinum">
                        {postItem.body.length > 65 ?  `${postItem.body.substring(0, 65)}...` : postItem.body}
                    </p>
                </CardContent>
